...
def Finder_IETE(time1, time_name1, start1):
    ...
    print('--- Mode recherche GFGDW', time1, time_name1, ' ---', flush=True)
    ...
    if ((B - A) > (B - C) and (B - C) > (D - C) and (D - C) > (D - E) and (D - E) > (F - E) and df['c'].values[-2] == lows['c'].iloc[-1]) or ((A - B) > (C - B) and (C - B) > (C - D) and (C - D) > (E - D) and (E - D) > (F - E) and df['c'].values[-2] == highs['c'].iloc[-1]):
